...
function render() {
    ...
    if (state.stage === 'START') {
        app.innerHTML = `
            <div class="bg-white p-8 rounded-3xl shadow-xl border-b-8 border-indigo-500 text-center">
                <h1 class="text-3xl font-black text-indigo-900 mb-8 uppercase">📚 Revisão 3.1</h1>
                <input type="text" id="nivel" placeholder="Nível (Ex: 9º ano)" class="w-full p-4 mb-4 border-2 rounded-2xl outline-none focus:border-indigo-500">
                ...
                <button onclick="gerarQuestaoIA()" class="w-full bg-indigo-600 text-white font-black py-5 rounded-2xl shadow-lg hover:bg-indigo-700 transition-all">🚀 GERAR DESAFIO</button>
            </div>`;
    } else if (state.stage === 'LOADING') {
        app.innerHTML = `<div class="p-12 text-center bg-white rounded-3xl shadow-xl"><div class="loader mb-6" style="border: 4px solid #f3f3f3; border-top: 4px solid #4f46e5; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite;"></div><h2 class="font-bold">CONECTANDO AO MODELO 3.1...</h2></div>`;
    } else if (state.stage === 'QUIZ') {
        const q = state.currentQuestion;
        app.innerHTML = `
            <div class="bg-white p-8 rounded-3xl shadow-xl border-b-8 border-indigo-400">
                <h2 class="text-2xl font-bold text-gray-800 mb-8">${q.pergunta}</h2>
                <div class="grid gap-4">${q.alternativas.map((alt, i) => `
                    <button onclick="selecionarResposta(${i})" class="w-full p-5 border-2 border-gray-100 rounded-2xl hover:bg-indigo-50 text-left flex items-center">
        ...
        app.innerHTML = `
            <div class="text-center space-y-6">
                <div class="bg-white p-8 rounded-3xl shadow-xl border-b-8 ${isCorrect ? 'border-green-500' : 'border-red-500'}">
                    ...
                    <p class="mt-4 text-gray-700">${q.explicacao}</p>
                </div>
